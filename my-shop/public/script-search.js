class SearchApp {
    constructor() {
        ...
        this.query = decodeURIComponent(window.location.pathname.split('/').pop());
        this.init();
    }
    ...
    async loadProducts() {
        try {
            const response = await fetch(`/api/products?search=${encodeURIComponent(this.query)}`);
            const data = await response.json();
            if (data.success) {
                this.products = data.products;
                document.getElementById('search-title').textContent = `Результати пошуку для "${this.query}"`;
            }
        } catch (error) {
    ...
    renderProducts() {
        ...
        container.innerHTML = this.products.length > 0
            ...
            `).join('')
            : '<p class="col-span-full text-center text-gray-500">Товари не знайдені</p>';
    }
    ...
    setupEventListeners() {
        ...
        document.getElementById('mobile-search-input').value = this.query;
    }
